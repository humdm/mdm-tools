#!/bin/bash
# ==========================================================
#        HUA QIANG BEI XIAO HU - MDM EXPERT SYSTEM (V21)
# ==========================================================
...
while true; do
    ...
    printf "${GRN}  ║          🌟 抖音/B站：华强北小胡    🌟 咸鱼：福田吴彦祖            ║${NC}\n"
    ...
    printf "    ${YLW}▶ 4)${NC} ${BLU}查看监管状态 (桌面模式查询增强)${NC}\n"
    ...
    case $opt in
        1) 
            ...
            printf "\n${GRN}★ 全部步骤已执行！重启后密码为: 1234 ★${NC}\n"
            printf "${YLW}★ 提示：进桌面后录入指纹并建议再次重启。★${NC}\n"
            sleep 3
            ;;
        2)
            find_disks
        ...
        4)
            echo -e "\n${CYN}>>> 正在深度扫描监管状态...${NC}"
            echo -e "${RED}⚠️  如需密码请直接输入${NC}"
            STATUS=$(sudo profiles show -type enrollment 2>&1)
            if echo "$STATUS" | grep -q "Error"; then
                echo -e "\n${GRN}✅ [结果]: 设备未受监管或屏蔽已生效 (报错即成功)${NC}"
            else
                echo -e "\n${YLW}[结果]:${NC}\n$STATUS"
            fi
            printf "\n${CYN}回车键返回主菜单...${NC}"
